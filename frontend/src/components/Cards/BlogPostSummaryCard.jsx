...
import { LuEye, LuHeart, LuTrash, LuTrash2 } from 'react-icons/lu'

export default function BlogPostSummaryCard({title, imgUrl, updatedOn, tags, likes, views, onClick, onDelete}) {
  return (
    <div className='flex items-start gap-4 bg-white p-3 mb-5 rounded-lg cursor-pointer group'
    onClick={onClick}
    >
        ...
        <div className='flex-1'>
            ...
            <div className='flex items-center gap-2.5 mt-2 flex-wrap'>
                ...
                <div className='flex items-center gap-2.5'>
                    {tags.map((tag, index)=>(
                        <div className='text-xs text-cyan-700 font-medium bg-cyan-100/50 px-2.5 py-1 rounded'
                         key={`tag_${index}`}>
                            {tag}
                        </div>
                    ))}
