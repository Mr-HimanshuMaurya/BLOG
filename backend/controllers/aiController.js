...
const generateBlogPost = async(req, res)=>{
    try{
        ...
        const prompt = `Wite a markdown-formatted blog post titt=led "${title}". Use a ${tone} tone. Include an introduction, subheadings, code examples if relevant, and a conclusion.`;
...
const generateBlogPostIdeas = async (req, res) => {
    try{
        ...
        res.status(500).json({
            message:"Failed to generate blog post idea",
            error: error.message,
        });
    }
}
